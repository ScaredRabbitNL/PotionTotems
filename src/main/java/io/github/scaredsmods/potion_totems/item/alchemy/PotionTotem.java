...
public class PotionTotem implements FeatureElement {
    public static final Codec<Holder<PotionTotem>> CODEC = PTRegistries.R_POTION_TOTEM.holderByNameCodec();
    public static final StreamCodec<RegistryFriendlyByteBuf, Holder<PotionTotem>> STREAM_CODEC = ByteBufCodecs.holderRegistry(PTResourceKeys.RK_R_POTION_TOTEM);
    @Nullable
    ...
    public static String getName(Optional<Holder<PotionTotem>> potionTotem, String descriptionId) {
        if (potionTotem.isPresent()) {
            String s = potionTotem.get().value().name;
            if (s != null) {
                return descriptionId + s;
            }
        }

        String s1 = potionTotem.flatMap(Holder::unwrapKey).map(p_331494_ -> p_331494_.location().getPath()).orElse("empty");
